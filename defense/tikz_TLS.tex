...
%	\node[left of= c4] {$\mathtt{accept}$};
	...
	\uncover<1-1>{
		\node[draw, below=30pt of midpoint,inner sep = 5pt,align=center] {$ck \gets \mathsf{tls.PRF}(ms,``\mathtt{key\ expansion}", n_C \Vert n_S)$};
	}
	
	\uncover<2->{
		\node[draw, below=30pt of midpoint,inner sep = 5pt,align=center] {$ck \gets \mathsf{tls.PRF}(ms,``\mathtt{key\ expansion}", n_C \Vert n_S)$\\$\textcolor{red}{ek} \gets \mathsf{tls.PRF}(ms,\textcolor{red}{``\mathtt{some\ other\ label}"}, n_C \Vert n_S)$};
	}
