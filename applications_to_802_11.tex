...
\begin{figure}
	...
	\includestandalone[width=1\textwidth,mode=build,center]{tikz_4WHS}
...
%Note that the fourth handshake message $m_4$ serves no cryptographic purpose and could safely have been omitted.
%However,
%to stay true to the actual 4WHS protocol, we leave it in.

Note that the error handling semantics of the 4WHS is different from protocols like TLS and SSH.
%In the latter,
%a session that receives a bad message will automatically go into the $\rejected$ state.
Specifically,
...
\begin{figure}
	\centerline{
		\hspace{-0.7em}\includestandalone[width=1\textwidth,mode=build]{tikz_CCM}
	}
